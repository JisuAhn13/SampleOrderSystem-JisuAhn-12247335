...
| 항목 | 출처 | 비고 |
|------|------|------|
| 시료 ID | `sample->getId()` | string |
| 시료명 | `sample->getName()` | |
| 재고 수량 | `sample->getStock()` | |
| 대기주문 | RESERVED 상태 주문 수량 합계 | `reservedDemand` |
| 상태 | 판단 결과 문자열 | 고갈 / 부족 / 여유 |

출력 예시:
```
   ID           이름           재고  대기주문  상태
   ══════════════════════════════════════════════════
   S001   SiC-MOSFET           10       5   ✔ 여유
   S002   GaN-HPA               3      10   ! 부족
   S003   InP-HBT               0       0   ✕ 고갈
```
